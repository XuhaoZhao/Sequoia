...
def backtest_strategy(db, stock_code, stock_name):
    ...
    买入条件：
    1. 当天收盘价在20日均线附近（MA20 * 0.98 <= close_price <= MA20 * 1.02）
       即股价在20日线上下2%范围内
       - 如果股价在20日线上方（close_price >= MA20）：允许买入
       - 如果股价在20日线下方（close_price < MA20）：必须当日上涨（close_price >= prev_close）
    2. 20日均线斜率必须向上且大于正斜率历史20%分位值
       （MA20_Slope > 0 且 MA20_Slope >= 正斜率历史20%分位值）
       注：只统计正向斜率，负斜率不参与计算

    卖出条件（满足任一即卖出）：
    1. 盘中5%止盈：最高价达到买入价5%，立即止盈
    2. 动态止损：
       - 如果买入日收盘是跌的（相比前一日），第二天止损1%
       - 如果买入日收盘是涨的（相比前一日），第二天止损2%
    3. 跌破5日线：收盘价 < MA5
    """
    # 从数据库读取日K数据
    ...
    # 计算历史20日均线斜率的20%分位值
    # 只使用正向斜率（> 0），负斜率不参与统计
    valid_slopes = df[df['MA20_Slope'] > 0]['MA20_Slope'].dropna()
    if len(valid_slopes) < 20:
        return None

    slope_20th_percentile = valid_slopes.quantile(0.2)
    ...
    for i in range(len(df)):
        ...
        if not holding:
            # 检查数据有效性
            if pd.isna(row['MA20']) or pd.isna(row['MA20_Slope']) or pd.isna(row['prev_close']):
                continue

            # 买入条件：
            # 1. 收盘价在20日线上下2%范围内（MA20 * 0.98 <= close_price <= MA20 * 1.02）
            # 2. 如果股价在20日线上方或正好在20日线上：允许买入
            #    如果股价在20日线下方：必须当日上涨
            # 3. 20日均线斜率向上且大于正斜率的20%分位值

            # 检查价格是否在20日均线附近2%范围内
            price_near_ma20 = (row['close_price'] >= row['MA20'] * 0.98 and
                               row['close_price'] <= row['MA20'] * 1.02)

            # 检查是否满足买入位置条件
            if row['close_price'] >= row['MA20']:
                # 股价在20日线上方或正好在20日线上：允许买入
                position_ok = True
            else:
                # 股价在20日线下方：必须当日上涨
                position_ok = row['close_price'] >= row['prev_close']

            if (price_near_ma20 and
                position_ok and
                row['MA20_Slope'] > 0 and
                row['MA20_Slope'] >= slope_20th_percentile):
                holding = True
        ...
        if holding:
            # 1. 优先检查盘中5%止盈（最高价判断）
            hit_take_profit = row['high_price'] >= buy_price * 1.05

            # 2. 检查动态止损（盘中条件单，使用最低价判断）
            hit_stop_loss = row['low_price'] <= stop_loss_price

            # 3. 检查跌破5日线（收盘判断）
            below_ma5 = row['close_price'] < row['MA5'] and not pd.isna(row['MA5'])

            # 判断卖出（优先级：5%止盈 > 止损 > 跌破5日线）
            should_sell = False
            sell_reason = ""
            sell_price = None
    ...
    # 统计各种卖出原因
    stop_loss_count = len([t for t in trades if t['sell_reason'] == "止损"])
    below_ma5_count = len([t for t in trades if t['sell_reason'] == "跌破5日线"])
...
def plot_results(results_df):
    ...
    fig.suptitle('20日均线动态止损策略回测结果', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('ma20_dynamic_strategy_results.png', dpi=300, bbox_inches='tight')
    print("\n图表已保存为 ma20_dynamic_strategy_results.png")
    plt.show()

def main():
    """主函数"""
    print("=" * 60)
    print("20日均线动态止损策略回测系统")
    print("=" * 60)
    print("\n策略说明：")
    print("买入条件：")
    print("  1. 当天收盘价在20日线上下2%范围内")
    print("     （MA20 * 0.98 <= close_price <= MA20 * 1.02）")
    print("     - 股价在20日线上方（含正好在20日线）：允许买入")
    print("     - 股价在20日线下方：必须当日上涨")
    print("  2. 20日均线斜率向上且大于正斜率历史20%分位值")
    print("     （MA20_Slope > 0 且 MA20_Slope >= 正斜率历史20%分位值）")
    print("     注：只统计正向斜率，过滤掉最小的20%弱趋势")
    print("\n卖出条件（优先级从高到低）：")
    print("  1. 盘中5%止盈：最高价达到买入价5%，立即止盈")
    print("  2. 动态止损：")
    print("     - 买入日收盘跌（相比前一日）：第二天止损1%")
    print("     - 买入日收盘涨（相比前一日）：第二天止损2%")
    print("  3. 跌破5日线：收盘价 < MA5")
    print("=" * 60)
    ...
    total_below_ma5 = results_df['below_ma5_count'].sum()
    print(f"\n卖出原因统计：")
    print(f"  触发止损: {total_stop_loss} 次 ({total_stop_loss/total_trades*100:.2f}%)")
    print(f"  跌破5日线: {total_below_ma5} 次 ({total_below_ma5/total_trades*100:.2f}%)")
    ...
    # 保存详细结果到CSV
    output_file = 'ma20_dynamic_strategy_detailed.csv'
    ...
    if all_losing_trades:
        ...
        excel_file = 'ma20_dynamic_losing_trades.xlsx'
