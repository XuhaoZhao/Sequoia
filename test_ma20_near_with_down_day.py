...
def backtest_strategy(db, stock_code, stock_name):
    ...
    卖出条件（优先级从高到低）：
    1. 5日线大于20日线5%，以收盘价止盈
    2. 持有期间如果收盘价跌破20日均线，以MA20价格止损卖出
    3. 最多持有5天，第5天以收盘价卖出
    """
    # 从数据库读取日K数据
    ...
    for i in range(len(df) - 5):  # 减5是因为最多持有5天
        ...
        if holding:
            ...
            # 检查止盈条件：5日线大于20日线5%
            if current_row['MA5'] > current_row['MA20'] * 1.05:
                sell_price = current_row['close_price']
                sell_date = current_row['datetime']
                sell_reason = f"第{hold_days}天止盈（MA5>MA20*5%）"
    ...
    take_profit_count = len([t for t in trades if '止盈' in t['sell_reason']])
...
def plot_results(results_df):
    ...
    fig.suptitle('MA20附近+5日线向上+20日线向上+阴线策略（持有5天+MA20止损+止盈）回测结果', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('ma20_near_down_day_hold5days_ma20stop_takeprofit_strategy_results.png', dpi=300, bbox_inches='tight')
    print("\n图表已保存为 ma20_near_down_day_hold5days_ma20stop_takeprofit_strategy_results.png")
    plt.show()

def main():
    """主函数"""
    print("=" * 60)
    print("MA20附近+5日线向上+20日线向上+阴线策略回测系统")
    print("=" * 60)
    print("\n策略说明：")
    print("买入条件：")
    ...
    print("\n卖出条件（优先级从高到低）：")
    print("  1. 5日线大于20日线5%，以收盘价止盈")
    print("  2. 持有期间如果收盘价跌破20日均线，以MA20价格止损")
    print("  3. 最多持有5天，第5天以收盘价卖出")
    print("=" * 60)
    ...
    total_take_profit = results_df['take_profit_count'].sum()
    print(f"\n卖出原因统计：")
    print(f"  触发止盈: {total_take_profit} 次 ({total_take_profit/total_trades*100:.2f}%)")
    ...
    # 保存详细结果到CSV
    output_file = 'ma20_near_down_day_hold5days_ma20stop_takeprofit_strategy_detailed.csv'
    ...
    if all_losing_trades:
        ...
        excel_file = 'ma20_near_down_day_hold5days_ma20stop_takeprofit_losing_trades.xlsx'
